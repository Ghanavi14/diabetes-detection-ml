...
def register():
    if request.method=='POST':
        #read the inputs from the registration form
...
file_uploaded=file_uploaded,columns=columns,data=data)
...
def train():
    ...
    try:
        ...
        rf_model = RandomForestClassifier(n_estimators=300,max_depth=20,min_samples_split=5,min_samples_leaf=2,max_features='sqrt',class_weight='balanced',random_state=42)
        rf_model.fit(X_train, y_train)

        if not os.path.exists('models'):
            os.makedirs('models')
        joblib.dump(rf_model, "models/models.pkl")
        # Evaluate model
